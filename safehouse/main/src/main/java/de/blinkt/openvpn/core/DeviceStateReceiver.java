...
public class DeviceStateReceiver extends BroadcastReceiver implements ByteCountListener, OpenVPNManagement.PausedStateCallback {
    ...
    private Runnable  runnable = new Runnable() {
        ...
        public void run() {
           /* SharedPreferences prefs = Preferences.getDefaultSharedPreferences(context);
            ...
            System.out.println("User pause thread comes true");
            editor.putBoolean("startedInternally", true);
            editor.commit();
            userPause(true);*/
        }
    };
    ...
    public void userPause(boolean pause) {
        if (pause) {
            ...
            System.out.println("wereConnected "+wereConnected);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ConnectivityManager.CONNECTIVITY_ACTION.equals(intent.getAction())) {
            ...
            if (screenOffPause) {
                ...
                System.out.println("Connected but screenOff so pause:- "+shouldBeConnected());
            }
            ...
            if (shouldBeConnected() != connected) {
                mManagement.resume();
                System.out.println("Connected but screenn so resume");
            }/*else if(isUserPaused() && prefs.getBoolean("startedInternally", false)){
                editor.putBoolean("startedInternally", false);
                editor.commit();
                mManagement.resume();
                System.out.println("Connected isUserPaused true so resume and boolen: "+prefs.getBoolean("startedInternally", false));
               *//* if(startedInternally) {
                ...
                mManagement.pause(getPauseReason());
                System.out.println("not called internally: ");
            }
